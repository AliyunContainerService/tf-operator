	...
	for index, podSlice := range podSlices {
		...
		if len(podSlice) > 1 {
			...
			if _, ok := spec.Template.Labels[TFPodGroupSettingLabel]; ok && CheckTFJobIsNotPending(tfjob) {
				isWorker0Type := rtype == tfv1beta2.TFReplicaTypeWorker && index == 0
				if isWorker0Type {
...
func getPodTTL(tfjob *tfv1beta2.TFJob) (time.Duration, error) {
...
func waitPodTTLReached(tfjob *tfv1beta2.TFJob, ttlDuration time.Duration) bool {
